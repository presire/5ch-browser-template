...
- 全板スレタイ検索（スレ一覧の検索欄から全板のスレタイを横断検索、Ctrl+Enter で実行、AND / OR / 除外 / "フレーズ" / @板名 で絞り込み）
...
- 一括巡回（お気に入り・最近読んだ・最近書き込んだスレをまとめて新着チェック、更新ありスレを太赤字表示・更新件数バッジ）
...
- X（Twitter）動画つきポストのカード内インライン再生（GIF は無音ループ再生）
- 本文中の `video.twimg.com` 動画直リンクをサムネイル付きでインライン再生（URL リンクは従来どおり保持）

### 書き込み

- 引用 / プレビュー / sage トグル / 文字数カウンタ
- 絵文字ピッカー（投稿・スレ立てフォームからフローティング別ウィンドウで起動、ドラッグ移動可）
...
- 自分宛のレス（自分の書き込みへの `>>` アンカー）を検出して Discord に通知
- 自分が書き込んだスレのみを定期巡回（巡回間隔は 1〜180 分で設定）
- 通知にはスレタイ / レス番号 / 本文 / 名前 / 日付 / ID を含み、該当レスへ直接リンク
- Discord ユーザー ID を設定するとメンション付きで送信（サーバーの通知設定が「@メンションのみ」でもスマホにプッシュが届く）
- 独立した通知設定パネル（ファイル → 通知設定）、テスト送信ボタン付き、入力は自動保存
- Ember の起動中のみ動作（PC のスリープ中・終了中は通知されない）
...
- レス選択時に該当レスへ自動スクロール（ON/OFF）
- レス本文の末尾に空行を追加（ON/OFF）
- 起動時に新バージョンを検出するとバージョン情報ダイアログを自動表示（バージョン毎に 1 回）
- ウィンドウ位置・サイズ・最大化状態の復元（Windows）
- 常に最前面トグル
- `5ch.net` → `5ch.io` 自動リダイレクト
- EXおろちつねる（bbspink.org/ex0ch）対応（5ch 互換プロトコル、bbsmenu 自動マージ、閲覧 + 書き込み、設定で表示/非表示切替）
