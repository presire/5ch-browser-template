...
### 27.4 実装反映状況（2026-03-07）
- 実装済み:
  - メニューバー/ツールバー/ステータスバー
  - 上部URLバー（入力値を投稿対象URLへ反映）
  - 3ペイン構成（板/スレ/レス）
  - レスビューアとDeveloper Toolsの分離（折りたたみ）
  - スレ右クリックメニュー枠（既読/未読）
- `live5ch_3.jpg` / `live5ch_4.jpg` 反映済み:
  - スレ一覧に補助列（取得レス/勢い/最終読込/最終書込）を追加
  - レス右クリックメニューの多項目化（引用/URLコピー/NG関連など）
  - ステータスバーに通信・認証指標（TS/US/API/Ronin/BE等）を追加
- 未実装:
  - 行情報列の実データ接続（現在はモック値）
  - キーボードショートカット
  - ペイン境界ドラッグによるサイズ変更
