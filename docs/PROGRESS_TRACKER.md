...
- [ ] 曖昧 NG — 自然文ルールをローカル LLM で判定する遅延 NG (N23)。クラウド判定モデル Jev は 2026-09-21 不採用。ローカル実行可能な Jev クローン Laya (BERT 系) は代替候補として記録済み (未検証)。同方式を GGUF 汎用モデルで実証した SemIf (旧 OpenJev) を参考に、既存 llama-cpp-2 基盤での実装案を BRUSHUP_PLAN に記載済み
