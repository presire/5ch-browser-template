...
**アイディア (見送り。再検討条件は BRUSHUP_PLAN N23)**
- [ ] 曖昧 NG — 自然文ルールをローカル LLM で判定する遅延 NG (N23)。**2026-09-21 見送り** (偽陽性率が実スレ未計測・CPU では速度と閾値が両立せず・遅延 NG の UX・対象ユーザーが薄い。GPU 限定案と Mac の速度見立ても BRUSHUP_PLAN に記録済み)。クラウド判定モデル Jev は 2026-09-21 不採用。ローカル実行可能な Jev クローン Laya (BERT 系) は代替候補として記録済み (未検証)。同方式を GGUF 汎用モデルで実証した SemIf (旧 OpenJev) を参考に、既存 llama-cpp-2 基盤での実装案を BRUSHUP_PLAN に記載済み
